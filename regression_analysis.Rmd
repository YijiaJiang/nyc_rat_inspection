...
=======
   ggpairs(
     title = "Correlations Between Key Predictors"
   ) +
>>>>>>> 09dfd02f5a7d2e92bc2b69916dbe5ecbdcf4b2a5
  scale_fill_discrete()
```
...
The full multiple linear regression model we constructed is $y_\_borough\_monthly\_cases= \beta_0+ \beta_1(inspectionMonth) + \beta_2(borough) + \beta_3(covid) + \beta_4(precipitaiton) + \\\beta_5(snowFall) + \beta_6(snowDepth) + \beta_7(temperature)$. The result shown that the p-value of inspectionMonth, precipitaiton, snowFall, and temperature were greater than 0.05, which had to be analyzed by stepwise regression method.
...
We eliminated those variables that are not statistically significant using stepwise regression: $y_\_borough\_monthly\_cases= \beta_0+ \beta_1(borough) + \beta_2(covid) + \beta_3(snowFall) + \beta_4(snowDepth)$
...
$log(borough\_monthly\_cases)= \beta_0+ \beta_1(inspectionMonth) + \beta_2(borough) + \beta_3(covid) + \beta_4(precipitaiton) + \\\beta_5(snowFall) + \beta_6(snowDepth) + \beta_7(temperature)$
...
$log(borough\_monthly\_cases)= \beta_0+ \beta_1(inspectionMonth) + \beta_2(borough) + \beta_3(covid) + \beta_4(precipitaiton)+ \\ \beta_5(snowDepth) + \beta_6(temperature)$
...
According to the plot, there was no obvious difference on RMSE between model 3 and model 4. However, model 2 had a relatively lower average value of RMSE compared with model 1, since three variables were dropped by stepwise elimination. Therefore model 2 fit better than model 1.
