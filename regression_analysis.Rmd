...
cases = rat %>% 
  ...
  distinct()
rat_tidy = rat %>% 
  select(borough, inspection_ym, prcp, snow, snwd, tmin, tmax) %>%
  mutate(borough = as.factor(borough), inspection_ym = as.factor(inspection_ym), 
...
original_lm = lm(borough_month_cases ~ inspection_ym + borough + prcp + snow + snwd + tmin + tmax, data = cases_borough_month) 
broom::tidy(original_lm) %>% 
  knitr::kable()
model_interact = lm(borough_month_cases ~ inspection_ym + borough + snow + prcp + snwd + tmin + tmax + tmin*snwd, data = cases_borough_month) 
broom::tidy(model_interact) %>% 
  knitr::kable()
```

```{r, message = FALSE, warning = FALSE,comment = FALSE}
step_lm = step(model_interact, direction = "backward")
broom::tidy(step_lm) %>% 
  knitr::kable()
```
