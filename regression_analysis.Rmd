...
# without covid
rat_covid = rat_binary %>% 
  filter(inspection_year == 2021)

model_logit_2021 = glm(inspection_result ~ borough + inspection_daytime + prcp_yn + snow_yn + snwd_yn + feeling, data = rat_binary, family = "binomial")
summary(model_logit_2021)
step(model_logit_2021, direction = "both", k = 2)
step()
```
...
```{r, message=FALSE, warning=FALSE}
# stepwise for AIC
step(model_logit_full, direction = "both", k = 2)

model_logit = glm(inspection_result ~ borough + inspection_daytime + covid_yn + prcp_yn + snow_yn + snwd_yn + feeling, data = rat_binary, family = "binomial")
