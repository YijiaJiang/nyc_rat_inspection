...
rat_tidy = rat %>% 
  select(borough, inspection_ym, inspection_month, inspection_daytime, prcp, snow, snwd, tmin, tmax) %>%
  mutate(borough = as.factor(borough), inspection_month = as.factor(inspection_month), 
         inspection_daytime = as.factor(inspection_daytime), prcp = as.numeric(prcp), 
         snow = as.numeric(snow), snwd = as.numeric(snwd),
...
model_linear_original = lm(borough_month_cases ~ inspection_month + borough + inspection_daytime + prcp + snow + snwd + tmin + tmax, data = cases_borough_month) 
...
rat_binary = rat %>% 
  select(result, borough, inspection_month, inspection_daytime, prcp, snow, snwd, tmin, tmax) %>%
...
model_logit = glm(inspection_result ~ inspection_month + borough + inspection_daytime + prcp + snow + snwd + tmin + tmax, data = rat_binary, family="binomial")
...
```{r, message = FALSE, warning = FALSE,comment = FALSE}
model_logit_new = glm(inspection_result ~ borough + inspection_daytime + prcp + snwd, data = rat_binary, family="binomial")
